...
export function MockInterviewPage() {
  ...
  useEffect(() => {
    ...
    if (question.question_number === 1) {
      speak(
        `${activeDomain.pages.interview.welcomeSpeech} ${questionText}`
      );
      return;
    }
    ...
    return (
      <section className="mx-auto max-w-2xl rounded-md border border-slate-200 bg-white p-8 text-center shadow-sm">
        ...
        <h1 className="mt-5 text-2xl font-bold text-slate-950">
          {activeDomain.pages.interview.emptyTitle}
        </h1>
        <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-600">
          {activeDomain.pages.interview.emptyDescription}
        </p>
        <Button className="mt-6" onClick={() => navigate("/onboarding")}>
          {activeDomain.pages.interview.emptyAction}
        </Button>
      </section>
    );
  }
